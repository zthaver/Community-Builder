...
const Blog = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" role="status" aria-live="polite">
        <div className="text-center">
          <Loader2Icon className="w-12 h-12 animate-spin text-blue-600 mx-auto mb-4" aria-hidden="true" />
          <p className="text-xl text-gray-600">Loading article...</p>
          <span className="sr-only">Please wait while the article loads</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <article className="max-w-4xl mx-auto px-4" aria-labelledby="article-title">
        {/* Back Button */}
        <Link 
          href="/blog"
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          {postData.blogImage?.formats?.large?.url ? (
            <div className="relative w-full h-80 md:h-96">
              <Image
                ...
                fill
                alt={`Image for article: ${postData.articleTitle}`}
          ...
          <div className="p-8 md:p-10">
            {/* Article Title */}
            <h1 id="article-title" className="text-3xl md:text-4xl font-bold text-gray-900 mb-6 leading-tight">
        ...
        {/* Comment Section */}
        <section className="bg-white rounded-2xl shadow-lg p-8 md:p-10 mt-8" aria-labelledby="comments-heading">
          <div className="flex items-center gap-3 mb-6">
            <MessageSquareIcon className="w-8 h-8 text-blue-600" aria-hidden="true" />
            <h2 id="comments-heading" className="text-2xl md:text-3xl font-bold text-gray-900">
              Share Your Thoughts
            </h2>
          </div>
          
          <form onSubmit={submitComment} className="mb-10" aria-label="Post a comment">
            <label htmlFor="comment-input" className="block text-lg font-semibold text-gray-700 mb-3">
              Write a comment:
            </label>
            <Textarea
              id="comment-input"
            ...
            />
            <p id="comment-hint" className="sr-only">Enter your comment about this article, then click Post Comment to submit</p>
            <Button
              type="submit"
            ...
            >
              {isSubmitting ? (
                ...
                <SendIcon size={24} aria-hidden="true" />
              )}
